...
public:
    ...
    inline void SetInELRegion(bool v) { fInELRegion = v; }
    inline bool InELRegion() const    { return fInELRegion; }
    void Initialization() override
    {
        ...
        numPMTs = 8;    // For coincidence calculation
        ...
        inGas = false;
        T_Kelvin = 164.;  // for liquid drift speed calculation
        p_bar = 2.14;     // gas pressure in units of bars, it controls S2 size
        //not sure what pressure should be

        // Data Analysis Parameters and Geometry
        dtCntr = 44.624;  // center of detector for S1 corrections, in usec.
        dt_min = 28.439;  // minimum. Top of detector fiducial volume
        dt_max = 60.809;  // maximum. Bottom of detector fiducial volume

        radius = 46.5;  // millimeters (fiducial rad)
        radmax = 46.5;  // actual physical geo. limit

        TopDrift = 105.2;
        anode = 115.2;
        gate = 103; //not sure what this value should be
        cathode = 49.2;
    ...
    //     return 0.;  // unitless, 1.000 at detector center
    // }

    double FitS2(double, double, LCE) override {
        // S2 PDE "on" only inside the EL region
        return InELRegion() ? 1.0 : 0.0;
    }

    bool InELRegion(double z_mm) const { return (z_mm > TopDrift - 0.5) && (z_mm <= TopDrift); }

    vector<double> FitTBA(double xPos_mm, double yPos_mm, double zPos_mm) override {
    ...
    double OptTrans(double xPos_mm, double yPos_mm, double zPos_mm) override {
        ...
        phoTravT += RandomGen::rndm()->rand_gauss(0.00, sig,false);  
            // the overall width added to photon time spectra by the
